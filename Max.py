...
with open('outds.csv', 'r') as csvfile:
	...
	Queried_ID    = '99990'    ### Queried Outlier
	...
	#####################         Outlier detection in subpopulations      #############################
			...
			if (pop_size!= 0):
				...
				Sal_outliers = np.ones(len(Sal_list)) 
				Outlier_value = max(Sal_arr) 
				for max_finder in range(0, len(ID_list)):
					if (Sal_arr[max_finder] == Outlier_value):              #### Outlier detection
						Sal_outliers[max_finder] = -1    		#### Outlier labeling	
	...
	############  Finding max subpopulation wiht least # of attribute values for outlier true answer  ###########
	...
	print '\n\nOutlier value is:' , Sub_pop_sorted[outlier_index][2], '\nOutlier features [i, j, pop_size, Score, ID] are =', \
	Sub_pop_sorted[outlier_index]
